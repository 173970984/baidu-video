...
const rules = {
	...
	//手机校验
	phone:function(val){
		  if(val && !(/^1[3-9][0-9]\d{8,8}$/.test(val))){
		     return '格式不正确'
		  }
		  return true
	},
	baijia:function(val){
		...
		if(val.indexOf("https://baijiahao.baidu.com/")!=0){
			
			return '必须为https://baijiahao.baidu.com/开头';
		}
		 return true
	},
	//密码相同校验
	passwordConfirm:function(val,param,form){
	...
	between: function(val, param) {
		if(param.length > 1) {
			if(Number(val) < Number(param[0]) || Number(val) > Number(param[1])) {
				return '应在' + param[0] + '~' + param[1] + '个字符之间'
			}
		}else if(val != param[0]){
			return '应等于' + param[0]
		}
